...
function Client()
{
    const arrClient = [{clientid:1,clientname:'Client 1 Pvt Ltd'},
    ...
    return(
        ...
        <button onClick={()=>setClientName("Client 1 ")}>Set Client Name</button>
        <div>
            {clientData.map((item,idx)=>{
                return(
                    <p key={idx}>{item.clientid}  {item.clientname}</p>
                )
            })}
        </div>
        <div>
